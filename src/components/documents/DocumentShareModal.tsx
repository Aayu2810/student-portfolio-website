...
export default function DocumentShareModal({ documentIds, onClose }: DocumentShareModalProps) {
    ...
    try {
      const response = await fetch('/api/share', {
        ...
        body: JSON.stringify({
          ...
          expires_in_hours: expiresIn ? parseInt(expiresIn) : null,
          max_access: maxAccess ? parseInt(maxAccess) : null,
        }),
      })
      ...
      if (response.ok) {
        ...
        const qrResponse = await fetch(
          `https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=${encodeURIComponent(data.share_url)}`
        )
  ...
  const downloadQR = () => {
    ...
    link.href = qrDataUrl
    link.download = 'document-share-qr.png'
    link.click()
  }
    ...
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-gray-900 border border-gray-800 rounded-2xl max-w-lg w-full p-6">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-2xl font-bold text-white">Share Documents</h2>
          <button
            onClick={onClose}
        ...
        <div className="space-y-4 mb-6">
          <div>
            <label className="block text-sm font-medium text-gray-300 mb-2">
              Sharing {documentIds.length} document{documentIds.length > 1 ? 's' : ''}
            </label>
            <p className="text-sm text-gray-400">
              Recipients will have access to view and download these documents
            </p>
          </div>
